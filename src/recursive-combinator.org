...
*** note
    ...
    - one must use recur to do recursive call
      fun scan the argument array recursively
      and replace the recur with the fun
...
*** note eva & apply
    ...
    - eva should be called in native function body
      it returns the retack_point back to the native function body
