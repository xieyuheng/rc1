...
(function() {
  var HASH_TABLE, HASH_TABLE_ENTRY, RETACK_POINT, STACK, _equal, abs, add, anp, antecedent_actual_length, apply, argack, array_p, asr, atom_p, call_with_indentation, car, cat, cat_get_indentation_array, cat_indentation, cdr, concat, cond, cons, div, drop, dup, empty, eq, equal, eva, eva_dispatch, eva_into, eva_out, eva_primitive_function, eva_with_map, factorial, function_p, get, gt, gteq, guard, ifte, in_browser, in_node, incat, into, length, log, lt, lteq, map, match, max, min, mod, mul, neg, nop, object_p, orp, orz, out, over, pow, repl, repl_with_map, retack, reverse, sad, set, string_p, sub, swap, tes, tuck, unify, unify_array, unify_dispatch, unit, va, ya;
  ...
  eva_dispatch = function(jo, retack_point) {
    if (function_p(jo)) {
      return eva_primitive_function(jo);
    } else if (jo === void 0) {
  ...
  eva_primitive_function = function(jo) {
  ...
  tes = function(array1, array2) {
    var cursor, result1, result2, success;
  ...
  tes([[1, 2, 3], [1, 2, 3], tes], [[4, 5, 6], [4, 5, 6], tes]);
  ...
  tes([[4, 5, 6], dup, 0, 0, set, dup, 1, 1, set, dup, 2, 2, set], [[0, 1, 2]]);
  ...
  tes([[1, 2, 3], dup, concat], [[1, 2, 3, 1, 2, 3]]);
  ...
  tes([[1, 2, 3], dup, reverse, concat, dup, length], [[1, 2, 3, 3, 2, 1], 6]);
  ...
  tes([2, 3, pow, 8, eq], [true]);

  tes([2, 3, pow, 8, equal], [true]);
  ...
  tes([[1], apply, [dup, dup], apply], [1, 1, 1]);
  ...
  tes([[[false], [321], [true], [123]], cond], [123]);
  ...
  match = function(sequent_array) {
    var antecedent, argument_array, index, new_retack_point, result_map, succedent;
    index = 0;
    while (index + 1 < sequent_array.length) {
      antecedent = sequent_array[index];
      succedent = sequent_array[index + 1];
  ...
  tes([666, 666, 1, [[2], [1, 2, 3], [666, 1], [4, 5, 6]], match], [666, [4, 5, 6], apply]);

  tes([1, 2, 3, [[1, va("2"), 4], [null], [1, va("2"), 3], [out("2"), out("2")]], match], [2, dup]);

  tes([1, 2, 3, [[1, va("2"), 4], [null], [1, va("2"), 3], [out("2")]], match], [2]);

  tes([1, 2, 3, [[1, va("2"), 4], [null], [1, va("2"), 3, guard([false])], [false, out("2")], [1, va("2"), 3, guard([true])], [true, out("2")]], match], [true, 2]);

  tes([1, 2, 3, [[1, va("2"), 4], [null], [1, va("2"), 3, guard([1, out("2"), gt])], [false, out("2")], [1, va("2"), 3, guard([1, out("2"), lt])], [true, out("2")]], match], [true, 2]);
  ...
  repl_with_map = function(array, map) {
    var base_cursor, first_retack_point, jo, retack_point;
    base_cursor = retack.cursor();
    first_retack_point = new RETACK_POINT(array);
    first_retack_point.local_variable_map = map;
  ...
  factorial = sad([[dup, 1, eq], [], [dup, 1, sub, factorial, mul], ifte]);

  repl([3, factorial, 6, factorial]);
