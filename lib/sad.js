...
(function() {
  var HASH_TABLE, HASH_TABLE_ENTRY, RETACK_POINT, STACK, argack, array_p, asr, atom_p, call_with_indentation, cat, cat_get_indentation_array, cat_indentation, eva, eva_dispatch, eva_into_local_variable, eva_outo_local_variable, eva_primitive_function, function_p, in_browser, in_node, incat, into, object_p, orz, outo, retack, sad, string_p, ya;
  ...
  object_p = function(value) {
    return (value instanceof Object) && !array_p(value);
  };
  ...
  eva_dispatch = function(jo, retack_point) {
    if (function_p(jo)) {
      ...
      return eva_outo_local_variable(jo._outo_local_variable, retack_point.local_variable_map);
    } else {
  ...
  eva_into_local_variable = function(array, local_variable_map) {
    var fn, j, len, name_string, results;
    array = array.reverse();
    ...
    for (j = 0, len = array.length; j < len; j++) {
      name_string = array[j];
      fn(name_string);
      results.push(local_variable_map.set(name_string, argack.pop()));
    }
    return results;
  };
  ...
  eva_outo_local_variable = function(array, local_variable_map) {
    ...
    for (j = 0, len = array.length; j < len; j++) {
      ...
      if (result === void 0) {
        results.push(orz("- in eva_outo_local_variable\n", "  meet undefined name : ", name_string));
      } else {
  ...
  (function() {
    var add, my_object, testing_sad;
    add = function(a, b) {
      return a + b;
    };
    testing_sad = sad([1, 2, 3]);
    my_object = {
      k1: "value k1 of my_object"
    };
    eva([1, 2, 3, add, add, sad([1, 2, 3]), add, add, testing_sad, add, add, my_object, "k1", ya]);
    asr(argack.pop() === my_object.k1);
    asr(argack.pop() === 6);
    asr(argack.pop() === 6);
    asr(argack.pop() === 6);
    asr(argack.cursor() === 0);
    return eva([1, 2, 3, into("1", "2", "3"), outo("1", "2", "3"), outo("1", "2", "3")]);
  })();
